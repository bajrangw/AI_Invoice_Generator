...
const AllInvoices = () => {
  ...
  const handleStatusChange = async (invoice) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(API_PATHS.INVOICE.UPDATE_INVOICES(invoice._id), updatedInvoice);
                  ...
                  <tr key={invoice._id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-4" onClick={(e) => e.stopPropagation()}>
                        ...
                        {invoice.status !== 'Paid' && (
                          <Button size="small" variant="ghost" onClick={() => handleOpenReminderModal(invoice._id)} title="Generate Reminders"><Mail className="w-4 h-4 text-blue-500" /></Button>
                        )}
                      </div>
                    </td>
                  </tr>
