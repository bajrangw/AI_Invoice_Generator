...
const CreateInvoice = ({ existingInvoice, onSave }) => {
  ...
  const generateNewInvoiceNumber = (invoices) => {
    const maxNum = invoices.reduce((max, ivn) => {
      const parts = ivn.invoiceNumber?.split("-");
      if (!parts || parts.length < 2) return max;
    ...
    if (existingInvoice) {
      ...
      const fetchInvoices = async () => {
        ...
        try {
          ...
          const newInvoiceNumber = generateNewInvoiceNumber(invoices);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-[100vh]">
      ...
      {/* Items Table */}
      <div className="bg-white border border-slate-200 rounded-lg shadow-gray-100 overflow-hidden">
                ...
                <tr key={index} className="hover:bg-slate-50">
                  <td className="px-2 sm:px-6 py-4">
                    <input
                      ...
                      className="w-full h-10 px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-blue-500"
